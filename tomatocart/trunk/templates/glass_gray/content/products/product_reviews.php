...
<div style="float: right; text-align: center; margin: 10px">
  ...
  <?php echo '<p>' . toc_draw_link_button(osc_href_link(basename($_SERVER['SCRIPT_FILENAME']), $osC_Product->getID() . '&' . osc_get_all_get_params(array('action')) . '&action=cart_add'), $osC_Language->get('button_add_to_cart')) . '</p>'; ?>
</div>
...
?>
    ...
    <span style="float: right;"><?php echo toc_draw_link_button(osc_href_link(FILENAME_PRODUCTS, 'reviews=new&' . $osC_Product->getID()), $osC_Language->get('button_write_review')); ?></span>

<?php
  }
?>
  <?php echo toc_draw_link_button(osc_href_link(FILENAME_PRODUCTS, $osC_Product->getID()), $osC_Language->get('button_back')); ?>
</div>
