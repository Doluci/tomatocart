    ...
    <?php
      ...
      if ($Qproducts->numberOfRows()) {
        ...
        while ($Qproducts->next()) {
          ...
          $content = '<div style="margin-top: 10px; float:left; width: 33%; text-align: center">' .
          ...
          if ($osC_Product->isSimple()) {
            ...
            $content .= '<span style="display:block; padding: 3px; text-align: center">' . $osC_Product->getPriceFormatedShow(true) . '</span>';
            ...
            $content .= osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qproducts->value('products_id')), $osC_Image->show($Qproducts->value('image'), $Qproducts->value('products_name')));
            $content .= '<span style="display:block; padding: 3px; text-align: center">' . $osC_Product->getPriceFormatedShow(true) . '</span>';
            $content .= osc_link_object(osc_href_link(FILENAME_PRODUCTS, $Qproducts->valueInt('products_id') . '&action=cart_add'), osc_draw_image_button('button_add_to_cart.png', $osC_Language->get('button_add_to_cart')));
          }
