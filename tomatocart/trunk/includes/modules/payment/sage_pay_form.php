  ...
  class osC_Payment_sage_pay_form extends osC_Payment {
    ...
    function process_button() {
      ...
      $crypt = array('VendorTxCode' => substr(date('YmdHis') . '-' . $osC_Customer->getID(). '-' . $osC_ShoppingCart->getCartID(), 0, 40),
                     ...
                     'FailureURL' => osc_href_link(FILENAME_CHECKOUT, 'process', 'SSL', null, null, true), 
      ...
      if ($this->not_null(MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_EMAIL)) {
      ...
      if ($this->not_null(MODULE_PAYMENT_SAGE_PAY_FORM_CUSTOMER_EMAIL_MESSAGE)) {
    ...
    function process() {
      ...
      if (isset($_GET['crypt']) && $this->not_null($_GET['crypt'])) {
        ...
        if ( ($return['Status'] != 'OK') && ($return['Status'] != 'AUTHENTICATED') && ($return['Status'] != 'REGISTERED') ) {
	        ...
	        if ( is_numeric($error_number) && $this->errorMessageNumberExists($error_number) ) {
	          $error_message = $this->getErrorMessage($error_number) . ' ' . $osC_Language->get('payment_sage_pay_form_error_general');
	        }
        ...
        if ( isset($return['VPSTxId']) ) {
          ...
          $_SESSION['sagepay_comments'] = 'Sage Pay Reference ID: ' . $return['VPSTxId'] . ($this->not_null($_SESSION['sagepay_comments']) ? "\n\n" . $_SESSION['sagepay_comments'] : '');
          osC_Order::process($orders_id, $this->order_status, $_SESSION['sagepay_comments']);
        }
      }
    }
